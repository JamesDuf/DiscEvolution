...
def run_model(config, cli_output_dir=None):
    ...
    if grid_params['type'] == 'winds-alpha':
        ...
        # For a dead zone, replace the converged scalar-alpha surrogate with the
        # real DeadZoneEOS: keep the Sigma profile/disc mass fixed by the loop
        # above, solve the (dead) accretion alpha for the target Mdot, then build
        # the spatial dead/active alpha & psi profile across the boundary.
        if eos_params["type"] == "DeadZoneEOS":
            ...
            eos = DeadZoneEOS(
                ...
                r1=eos_params.get("r1"),
                r_floor=eos_params.get("r_floor"),
                t0=eos_params.get("t0"), 
                t1=eos_params.get("t1"),
                t_initial_yr=sim_params["t_initial"],
                kappa=kappa, 
                e_rad=e_rad, 
        ...
        if eos_params["type"] == "IrradiatedEOS":
            alpha_dead = eos_params["alpha_SS_dead"]
            psi_dead = eos_params["psi_dead"]

            # Rebuild the EOS using exactly the dead-zone inner-region values
            eos = IrradiatedEOS(
                star,
                alpha_t=alpha_dead,
                kappa=kappa,
            ...
            psi_profile = np.full_like(grid.Rc, psi_dead, dtype=float)
            lambda_DW = 1.0 + 1.0 / (
                2.0 * (1.0 - e_rad) * (3.0 / psi_profile + 1.0)
            )
                        
            # Not used but needed for HDF5 saving not to crash
            alpha_SS_dead = eos_params["alpha_SS_dead"]
            alpha_SS_active = alpha_SS_dead
            psi_active = psi_dead
            alpha_DW = psi_dead * alpha_SS_dead
    ...
    if (alpha_SS_dead > 0.1) or (alpha_SS_dead<1.e-5):          
        ...
        with h5py.File(outfile, "w") as h5f:
            ...
            h5f.create_dataset("psi_R", shape=(0, nR), maxshape=(None, nR), dtype="f8")
            h5f.create_dataset("P", shape=(0, nR), maxshape=(None, nR), dtype="f8")
            h5f.create_dataset("rho_mid", shape=(0, nR), maxshape=(None, nR), dtype="f8")
            h5f.create_dataset("nu", shape=(0, nR), maxshape=(None, nR), dtype="f8")
            h5f.create_dataset("H", shape=(0, nR), maxshape=(None, nR), dtype="f8")
            nSt = np.asarray(disc.Stokes()).shape[0]
            h5f.create_dataset("St", shape=(0, nSt, nR), maxshape=(None, nSt, nR), dtype="f8")
            ...
            if 0.0 not in sim_params['t_interval']:
                ...
                for name, arr in [
                    ...
                    ("alpha_R", disc.alpha),
                    ("psi_R", np.broadcast_to(np.asarray(getattr(disc._eos, "_psi", 0.0), dtype=float), (nR,))),
                    ("P", disc.P),
                    ("rho_mid", disc.midplane_gas_density),
                    ("nu", disc.nu),
                    ("H", disc.H),
                    ("St", np.asarray(disc.Stokes())),
                    ("Sigma_grain_size", disc.grain_size[0]),
                                    ]:
                    d = h5f[name]
                    d.resize(1, axis=0)
                    d[0, :] = arr
                ...
                # Dead-zone radius (scalar, aligned with time_snap)
                _R_dz_val = getattr(disc._eos, "_R_dz", np.nan)
                ...
                h5f["B_xe"].resize(1, axis=0)
                h5f["B_xe"][0, :] = B_xe0
                h5f["C_xe"].resize(1, axis=0)
                h5f["C_xe"][0, :] = C_xe0
                h5f["D_xe"].resize(1, axis=0)
                h5f["D_xe"][0, :] = D_xe0
            ...
            if transport_params['gas_transport'] and wind_params["on"]:
                ...
                psi_profile = np.asarray(disc._eos._psi, dtype=float)
                if psi_profile.ndim == 0:
                    psi_profile = np.full_like(disc.R, float(psi_profile), dtype=float) # Replaces line above
            ...
            for ti in times:
                ...
                # Dead-zone / disc-structure diagnostics
                h5f["alpha_R"].resize(s + 1, axis=0);               h5f["alpha_R"][s, :]     = disc.alpha
                h5f["psi_R"].resize(s + 1, axis=0);                 h5f["psi_R"][s, :]       = np.broadcast_to(np.asarray(getattr(disc._eos, "_psi", 0.0), dtype=float), (nR,))
                h5f["P"].resize(s + 1, axis=0);                     h5f["P"][s, :]           = disc.P
                h5f["rho_mid"].resize(s + 1, axis=0);               h5f["rho_mid"][s, :]     = disc.midplane_gas_density
                h5f["nu"].resize(s + 1, axis=0);                    h5f["nu"][s, :]          = disc.nu
                h5f["H"].resize(s + 1, axis=0);                     h5f["H"][s, :]           = disc.H
                h5f["St"].resize(s + 1, axis=0);                    h5f["St"][s, :, :]       = np.asarray(disc.Stokes())
